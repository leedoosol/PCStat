...
FuncType log_io_with_pc = NULL;

int set_log_io_with_pc (FuncType fn)
{
	log_io_with_pc = fn;
	return 0;
}
EXPORT_SYMBOL(set_log_io_with_pc);

int log_syscall_with_pc(unsigned int fd, struct file *file, unsigned int count,
			 unsigned int type, unsigned long oldrsp, loff_t pos, ktime_t start, ktime_t end)
{
	if (fd >= 2)
	{
		...
		{
			log_io_with_pc(fd, file, count, type, oldrsp, pos, start, end);
		}
	}
